...
export class NewUserService { 
    ...
    constructor(@InjectModel('NewUser') private readonly newUserModel: Model<NewUserDocument>) {}

    private async sendWelcomeEmail(username: string, email: string): Promise<void> {
    ...
    async uploadFiles(files: Array<Express.Multer.File>, id: string) {
        try {
            ...
            return await this.newUserModel.findByIdAndUpdate(id, newUser, {new: true})
    ...
    async update(id: string, updateNewUserDto: CreateNewUserDto) {
        const updatedUser = await this.newUserModel.findByIdAndUpdate(id, updateNewUserDto, {new: true})
